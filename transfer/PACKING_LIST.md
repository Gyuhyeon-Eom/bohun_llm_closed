...
## 0. 챙기기 전에 — 내부망 API로 해결돼서 "반입이 필요 없는" 것들

| 항목 | 내부망 제공 여부 확인 | 제공되면 | 안 되면 |
|---|---|---|---|
| **생성 LLM** | FabriX(OpenAI 호환 API) 개통 여부 | **exaone·Ollama 반입 불필요 (−5.5GB)** — `LLM_BACKEND=openai` + `FABRIX_ENDPOINT/KEY/MODEL` 환경변수 3개면 끝, 코드 수정 없음 | 아래 exaone·Ollama 반입 (임시 대역) |
| **PPP존 데이터** (유사사례 원문·통합보훈시스템 조회) | 내부 API 연계 예정 | 반입물 없음 (이미 목록에서 제외됨) | — |
| **임베딩** | FabriX가 임베딩 API도 제공하는지 | 제공 시 bge-m3(2.3GB) 대체 가능 — 단, 어댑터 코드 소폭 추가 필요(현재는 로컬 bge-m3 전제) | bge-m3 반입 (기본 설계) |

→ **반입 전 T/F에 확인할 질문 두 개**: ① FabriX 개통·규격(엔드포인트/키/모델명), ② 임베딩 API 제공 여부.
①이 확정이면 아래 목록에서 exaone·Ollama 줄을 통째로 건너뛰어도 된다.
...
| ☐ | **exaone3.5 LLM 모델** ※FabriX 개통 시 불필요 | 직링크 없음 — 온라인 PC에 Ollama 설치 후 `ollama pull exaone3.5:7.8b` → `~/.ollama/models` 폴더 통째 복사 | 4.8GB |
...
| ✔ | 항목 | 받는 곳 | 크기 |
|---|---|---|---|
| ☐ | **OS 패키지 묶음** (python3·pip·venv·PostgreSQL16·빌드도구, 의존성 포함 .deb 전체) | 링크 다발이 아니라 명령 1줄 — 서버와 **같은 버전** 우분투에서: `apt-get install --download-only -o Dir::Cache::archives=$PWD/pgpkgs -y python3 python3-venv python3-pip postgresql-16 postgresql-client-16 postgresql-server-dev-16 build-essential` | ~200MB |
| ☐ | **pgvector 소스** | https://github.com/pgvector/pgvector/archive/refs/tags/v0.8.0.tar.gz | 1MB |
| ☐ | **Ollama 리눅스 본체** ※FabriX 개통 시 불필요 | https://ollama.com/download/ollama-linux-amd64.tgz | ~1GB |
...
| ☐ | **Ollama 윈도우** (개발 PC에서 LLM 돌릴 경우 ※FabriX 개통 시 불필요) | https://ollama.com/download/OllamaSetup.exe | ~700MB |
...
② Git·IntelliJ·(Ollama) 설치 프로그램 실행
...
## USB 구성 예시 (총 ~10GB, 32GB 권장)

```
USB/
├─ bohun_llm_closed/        ← 레포 ZIP 풀어둔 것
├─ wheels/linux/  wheels/windows/
├─ models/bge-m3/           ├─ ollama/models/ (exaone 블롭)
├─ pgpkgs/ (*.deb)          ├─ pgvector-0.8.0.tar.gz
├─ installers/ (python·git·intellij·ollama exe들)
